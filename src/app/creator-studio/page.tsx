...
export default function CreatorStudioPage() {
    ...
    return (
        <div className="min-h-screen bg-black text-white relative flex flex-col md:flex-row overflow-hidden">
            ...
            {/* Mobile Tab Header */}
            <div className="md:hidden bg-white/5 border-b border-white/10 backdrop-blur-md px-4 py-3 flex items-center justify-between sticky top-0 z-20 flex-shrink-0">
                <div className="flex items-center gap-2 text-j-gold font-black tracking-widest text-xs sm:text-sm uppercase">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-xl transition-all font-bold text-xs ${activeTab === 'overview' ? 'bg-j-gold text-black' : 'text-gray-400 bg-white/5 hover:text-white'}`}
                    >
                        Overview
                    </button>
                    <button
                        onClick={() => setActiveTab('curation')}
                        className={`px-3 py-1.5 rounded-xl transition-all font-bold text-xs ${activeTab === 'curation' ? 'bg-amber-500 text-black' : 'text-gray-400 bg-white/5 hover:text-amber-500'}`}
                    >
                        Curation
                    </button>
                </div>
            ...
            {/* Premium Sidebar */}
            <div className="hidden md:flex w-64 bg-white/5 border-r border-white/10 backdrop-blur-xl relative z-10 flex-col h-[calc(100vh-3.5rem)]">
                <div className="p-6">
            ...
            {/* Main Content Area */}
            <div className="flex-1 relative z-10 h-full md:h-[calc(100vh-3.5rem)] overflow-y-auto overflow-x-hidden">
                <div className="p-4 sm:p-8 max-w-7xl mx-auto">
                    ...
                    {activeTab === 'overview' && (
                        <div className="space-y-6 sm:space-y-8 animate-fade-in-up">
                            {/* Header */}
                            <div>
                                <h1 className="text-2xl sm:text-4xl font-black mb-2">Welcome Back, Creator</h1>
                                <p className="text-gray-400 text-sm sm:text-lg">Manage the platform, and upload premium content.</p>
                            </div>
                    ...
                    {activeTab === 'curation' && (
                        <div className="space-y-8 sm:space-y-12 animate-fade-in-up pb-20">
                            <div>
                                ...
                                <p className="text-gray-400 text-sm sm:text-lg">Manage homepage layouts and algorithmic behaviors.</p>
                            </div>

                            {/* Master Killswitch */}
                            <div className="bg-gradient-to-br from-zinc-900 to-black border border-white/10 rounded-3xl p-6 sm:p-8 relative overflow-hidden group">
                                ...
                                <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 sm:gap-8">
                                    ...
                                    <button
                                        ...
                                        className={`w-full lg:w-auto shrink-0 flex items-center justify-center gap-3 px-6 py-3.5 sm:px-8 sm:py-4 rounded-2xl font-black uppercase tracking-widest transition-all duration-300 shadow-xl ${
                                            isAlgorithmPaused 
                            ...
                            {isCurationLoading ? (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="bg-white/5 border border-white/10 rounded-3xl p-4 sm:p-6">
                                        <h3 className="text-lg sm:text-xl font-black text-white mb-6 border-l-4 border-j-gold pl-3">Featured Hero Carousel</h3>
                                        <p className="text-xs sm:text-sm text-gray-400 mb-6 italic">These videos appear at the very top of the homepage in the giant slider.</p>
                                                ...
                                                curatedVideos.featured.map(video => (
                                                    <div key={video.id} className="flex items-center justify-between bg-black/40 rounded-xl p-3 border border-white/5 group">
                                                        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                                                            ...
                                                            <div className="truncate font-bold text-xs sm:text-sm text-gray-200">{video.title}</div>
                                                        </div>
                                                        <button 
                                    ...
                                    {/* Trending List */}
                                    <div className="bg-white/5 border border-white/10 rounded-3xl p-4 sm:p-6">
                                        <h3 className="text-lg sm:text-xl font-black text-white mb-6 border-l-4 border-red-500 pl-3">Curated Trending Rail</h3>
                                        <p className="text-xs sm:text-sm text-gray-400 mb-6 italic">These videos are forced into the &quot;Trending Now&quot; rail. If algorithm is paused, ONLY these appear.</p>
