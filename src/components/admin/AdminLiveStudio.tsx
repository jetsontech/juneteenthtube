...
const translations = new Map<string, string>([
    ...
    ["actions", "Actions"],
    ["noChannelsFoundMatching", "No channels found matching "],
    ["original", "Original"],
...
export function AdminLiveStudio() {
    ...
    useEffect(() => {
        fetchChannels(); // eslint-disable-line react-hooks/set-state-in-effect
    }, []);
    ...
    const moveChannel = async (index: number, direction: 'up' | 'down') => {
        ...
        // Swap their array positions using safe .at() and .splice() to avoid bracket notation
        const temp = newChannels.at(index);
        const targetVal = newChannels.at(targetIndex);
        if (temp !== undefined && targetVal !== undefined) {
            newChannels.splice(index, 1, targetVal);
            newChannels.splice(targetIndex, 1, temp);
        }
    ...
    const handleAddChannel = async () => {
        ...
        try {
            ...
            const channelData = {
                id: safeUUID(), // Fallback if DB doesn't gen
        ...
        <div className="space-y-6">
                    ...
                    <button
                        onClick={async () => {
                            if (confirm("This will ingest and schedule all legacy vault videos for 'J-Tube Originals'. Proceed?")) {
                                try {
                                    const res = await fetch('/api/admin/vault-rotation', { 
                                        method: 'POST',
                                        headers: session?.access_token ? { 'Authorization': `Bearer ${session.access_token}` } : {}
                                    });
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-white/[0.03] text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">
                        <tr>
                            <th className="px-8 py-4">{t("channelDetails")}</th>
                            <th className="px-8 py-4">{t("statusAndType")}</th>
                    ...
                    <tbody className="divide-y divide-white/5">
                        {isLoading ? (
                            ...
                            <tr>
                                <td colSpan={3} className="px-8 py-12 text-center text-white/50">
                                    {t("noChannelsFoundMatching")}&quot;{searchQuery}&quot;
                                </td>
                            </tr>
                        ) : (
                                            ...
                                            <div className="min-w-0 max-w-xs md:max-w-md">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {channel.is_internal_vod && (
                                                        <span className="bg-gradient-to-r from-red-600 to-amber-600 text-[9px] px-1.5 py-0.5 rounded font-black uppercase tracking-widest text-white shrink-0 shadow-[0_0_10px_rgba(220,38,38,0.5)]">{t("original")}</span>
                                                    )}
                                                </div>
            ...
            {isAddModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm shadow-[0_0_60px_rgba(0,0,0,0.8)]">
                    <div className="w-full max-w-lg bg-zinc-950/60 backdrop-blur-2xl border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
                        ...
                        <div className="space-y-5">
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">{t("channelName")}<span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">{t("streamUrl")}<span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">{t("logoUrl")}</label>
                                <input
                                    type="text"
