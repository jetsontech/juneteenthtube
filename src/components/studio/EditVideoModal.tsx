...
const translations = new Map<string, string>([
    ...
    ["categoryCannotBeChanged", "Category cannot be changed after upload."],
    ["cancel", "Cancel"]
]);
...
export function EditVideoModal({ video, isOpen, onClose }: EditVideoModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4 animate-in fade-in duration-300">
            <div className="bg-[#1a1a1a]/60 backdrop-blur-2xl border border-white/10 rounded-[32px] w-full max-w-2xl overflow-hidden shadow-2xl shadow-black/50">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-black text-white uppercase tracking-tighter">{t("editVideoDetails")}</h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-4">
                            <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">{t("videoThumbnail")}</label>
                            <div
                            ...
                            >
                                {thumbnailPreview ? (
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-600">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest">{t("selectImage")}</span>
                                    </div>
                                )}
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center transition-all backdrop-blur-[2px]">
                                    ...
                                    <span className="text-[10px] font-black text-white uppercase tracking-widest bg-j-gold/80 px-4 py-2 rounded-full">{t("uploadNew")}</span>
                                </div>
                            </div>
                            <input
                                ref={fileInputRef}
                        ...
                        <div className="space-y-6">
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-2">{t("videoTitle")}</label>
                                <input
                                    type="text"
                                    value={title}
                            ...
                            <div>
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-2">{t("platformCategory")}</label>
                                <div className="px-4 py-3 bg-white/[0.03] border border-white/5 rounded-xl text-sm text-gray-400 font-medium">
                                    {video.category || "General"}
                                </div>
                                <p className="mt-2 text-[10px] text-gray-600 italic">{t("categoryCannotBeChanged")}</p>
                            </div>
                        </div>
                    </div>
