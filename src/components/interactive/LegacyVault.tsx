...
import { ShieldCheck, Play, UserCheck, Film, ArrowRight } from "lucide-react";
...
export const VAULT_ITEMS: LegacyItem[] = [
    {
        id: "1",
...
export function LegacyVault() {
    ...
    // Stateful vault items to allow dynamic verification updates from Admin dashboard
    const [vaultItems, setVaultItems] = useState<LegacyItem[]>(VAULT_ITEMS);

    const [prevActiveItem, setPrevActiveItem] = useState<LegacyItem | null>(null);
    ...
    React.useEffect(() => {
        const loadOverrides = () => {
            try {
                ...
                if (saved) {
                    const overrides = JSON.parse(saved) as Record<string, boolean>;
                    setVaultItems(prev => 
                        prev.map(item => ({
                            ...item,
                            isElderVerified: overrides[item.id] !== undefined ? overrides[item.id] : item.isElderVerified
                        }))
                    );
                } else {
                    // Initialize overrides in localStorage if not present
                    const initialOverrides: Record<string, boolean> = {};
                    VAULT_ITEMS.forEach(item => {
                        initialOverrides[item.id] = item.isElderVerified;
                    });
                    localStorage.setItem("jtube_vault_elder_verified", JSON.stringify(initialOverrides));
                }
            } catch (e) {
                console.error("Failed to parse vault elder verification overrides", e);
            }
        };
        ...
        const handleStorage = (e: StorageEvent) => {
            if (e.key === "jtube_vault_elder_verified") {
                loadOverrides();
            }
        };

        // Listen for custom events (within the same tab/window)
        const handleCustomSync = () => {
            loadOverrides();
        };

        window.addEventListener("storage", handleStorage);
        window.addEventListener("jtube_vault_sync", handleCustomSync);

        return () => {
            window.removeEventListener("storage", handleStorage);
            window.removeEventListener("jtube_vault_sync", handleCustomSync);
        };
    }, []);
    ...
    return (
        <div className="min-h-screen bg-transparent text-white pb-32">
            ...
            {activeItem && (() => {
                const currentItem = vaultItems.find(vi => vi.id === activeItem.id) || activeItem;
                return (
                    <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4 sm:p-12 overflow-y-auto animate-in fade-in duration-300">
                        <div className="max-w-4xl w-full bg-[#0a0a0a] border border-white/10 rounded-3xl overflow-hidden shadow-2xl relative flex flex-col md:flex-row my-auto max-h-[90vh] md:max-h-none overflow-y-auto md:overflow-visible">
                            <button
                                onClick={() => setActiveItem(null)}
                                className="absolute top-4 right-4 z-10 p-2 bg-black/50 rounded-full hover:bg-white/20 transition-colors cursor-pointer"
                            >
                            ...
                            <div className="w-full md:w-1/2 p-5 sm:p-8 flex flex-col justify-center">
                                <div className="flex items-center space-x-2 text-yellow-600 mb-2 sm:mb-4 text-xs font-black uppercase tracking-widest">
                                    <Film className="w-4 h-4" />
                                    <span>{currentItem.category}</span>
                                </div>
                                <h2 className="text-2xl sm:text-3xl md:text-4xl font-black mb-3 sm:mb-4 leading-tight">{currentItem.title}</h2>
                                <p className="text-gray-400 text-xs sm:text-sm leading-relaxed mb-4 sm:mb-8">
                                    A curated selection from the {currentItem.source} collection. Restored and digitized for the CultureQuest archives. verified by community elders for historical accuracy.
                                </p>
                                ...
                                <div className="flex items-center space-x-4 mb-4 sm:mb-8">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-white font-mono">{currentItem.year}</span>
                                    </div>
                                    <div className="w-px h-8 bg-white/10" />
                                    <div className="flex flex-col">
                                        <span className="text-[10px] uppercase text-gray-600 font-bold">Verified By</span>
                                        <span className="text-white">{currentItem.isElderVerified ? "Elder Council" : "Pending"}</span>
                                    </div>
                                </div>

                                <button
                                    onClick={() => setIsPlaying(true)}
                                    className="w-full bg-white text-black py-3 sm:py-4 rounded-xl font-black uppercase tracking-widest hover:bg-yellow-500 transition-colors flex items-center justify-center space-x-2 cursor-pointer text-xs sm:text-sm"
                                >
